...
const proxyIPs = ['103.167.150.212'];
	...
	userIDArray.forEach((userID) => {
		...
		if (!hostName.includes('pages.dev')) {
			...
			portArray_http.forEach((port) => {
				const commonUrlPart_http = `:${port}?encryption=none&security=none&fp=random&type=ws&host=${hostName}&path=%2Fvless-bodong#${hostName}-HTTP-${port}`;
		...
		portArray_https.forEach((port) => {
			const commonUrlPart_https = `:${port}?encryption=none&security=tls&sni=${hostName}&fp=random&type=ws&host=${hostName}&path=%2Fvless-bodong#${hostName}-HTTPS-${port}`;
